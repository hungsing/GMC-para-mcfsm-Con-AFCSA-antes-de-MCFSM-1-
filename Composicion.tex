...
\begin{itemize}
    ...
    \item $\hat{\Delta}= \{\langle q_i,\hat{\sigma},q_j,\rangle | q_i,q_j \in Q, \hat{\sigma} \in \delta \}$ a la relación de transición de $\hat{\mathcal{A}}$. Un estado de $\hat{Q}$ es alcanzable desde otro si existe una transición de comunicación interna en el autómata $\mathcal{A}$ que conecta un estado de la clase de equivalencia $[q]$ con uno de la $[q']$
...
\begin{tikzpicture}[->, thick][scale=0.20]
 ...
 \node[state] (q_2) [right = 3cm of q_1 ] {$q_{110},\epsilon, \epsilon$};
...
\begin{prop}[Equivalencia de semánticas] Dado un conjunto de weak deterministic AFCA $\{\mathcal{A_i}\}_{i \in I}$ sin transiciones de comunicación interna y su composición $\mathcal{A}$. Llamamos $\hat{\mathcal{A}}$ el sistema de transición etiquetado (LTS) resultante de la proyección $\Tau(\mathcal{A})$. Sea $\{C_i\}_{i \in I}$ el communicating system conformado por las mCFSM correspondientes al conjunto autómatas. Llamamos $C$ al LTS que representa la semántica de $\{C_i\}_{i \in I}$. Decimos que $\hat{\mathcal{A}}$ y $C$ son bisimilares.

\begin{proof}Primero necesitamos ver que para cada estado de $\hat{\mathcal{A}}$ existe un estado equivalente en $C$, y viceversa. Es decir $\forall q \in \hat{Q} p \in Q_C$.
