...
El problema es que los I/O no representan bien la comunicación asincrónica. Las operaciones de comunicación externa sincronizan a todos los participantes del sistema. Es decir que si dos autómatas componentes tienen una misma transición, se ejecutan en simultáneo. Esto va en contra de la idea de asincronicidad que buscamos.
...
\item Sea una cola no vacía $b= [m_1,m_2,\ldots,m_n]$ al aplicarle la operación $b \ll m$ queda $b= [m_1, m_2,\ldots,m_n,m]$ del mismo modo al aplicarle $b \gg m_1$ queda $b= [m_2,\ldots,m_n,m]$
\end{itemize}
\end{definition}
...
\item $B \subseteq \{b_{{pq}_n} \ | \ pq \in \mathcal{P}^2, n \in \mathbb{N}, p \neq q \}$ es un conjunto finito de buffers (i.e. colas, ver Def.~\ref{def:cola}),
\item $\mathcal{C} \subseteq \{pq_n \ | \ pq \in \mathcal{P}^2, n \in \mathbb{N}, p \neq q \}$ es un conjunto de canales tales que $B \cap \mathcal{C} = \emptyset$, y hay al menos un canal en cada sentido entre cada par de participantes, 
...
\end{definition}
La intuicion detrás de las componentes de un AFCA es la siguiente:

\begin{itemize}
    \item $\Sigma$ es el conjunto de todas las etiquetas del autómata y lo dividimos en tres subconjuntos disjuntos de la forma $\Sigma_{\mathit{Int}}$, $\Sigma_{\mathit{Ex}}$ y $\Sigma_{\mathit{Buff}}$ que corresponden a las acciones de procesamiento interno, las de comunicación externa con otros participantes y las de comunicación interna vía buffers.
    
    \item $\Sigma_{Ex}$ es el conjunto de etiquetas correspondientes a la comunicación externa del autómata. Como la comunicación es dirigida punto a punto, cada etiqueta incluye a dos participantes $p_1, p_2 \in \mathcal{P}$ (emisor y receptor) y un canal $c \in \mathcal{C}$. Necesariamente en todas las etiquetas vale que $p_1$ y $p_2$ son distintos y entre cada par de participantes hay al menos un canal en cada sentido de la comunicación.
    
    \item $\Sigma_{Buff}$ es el conjunto de etiquetas de acciones de comunicación interna vía buffers. Se especifican las acciones de encolar y desencolar como $b \ll m$, $b \gg m$, respectivamente, donde $b \in B$ y $m$ es el mensaje que se inserta/extrae del mismo.
    
    \item $\delta$ es el conjunto de transciciones que se compone de $\delta_{Int}$, las acciones internas, $\delta_{Ex}$ las transiciones de comunicación con otros agentes y $\delta_{Buf}$ las acciones de buffer.

    \item $\delta_{Ex}$ se compone de dos tipos de acciones que representan el intercambio de mensajes $m \in \mathcal{M}$ entre dos autómatas a través de un canal de comunicación $c \in \mathcal{C}$. Las acciones de entrada o input representan la recepción de un mensaje de algún proceso externo al correspondiente al autómata, denotadas \textit{In(c,m)}. Las acciones de output o salida que representan el envío de un mensaje y se denotan \textit{Out(c,m)}. Estas acciones son la parte externa de la comunicaión asincrónica. Como los canales son unidireccionales decimos que para todo par de participantes existen una cantidad finita de pares de canales.

    \item $\delta_{Buff}$ es la relación de transición de comunicación interna mediante buffers.
    
    \item Formalmente, F $\subseteq$ Q. Es el conjunto de estados finales, los posibles estados a los que una ejecución llega y es aceptada como válida. Para los autómatas asincrónicos pedimos también que al llegar a este estado final los buffers se encuentren vacíos. Para esto definimos lo siguiente:
\end{itemize}
...
Una configuración instantánea del autómata como $\langle q, \Omega \rangle \in Q \times \{ (m_b)_{b \in B} | m_b \in \mathcal{M}^* \}$ donde:
\begin{itemize}
...
\item $\langle q_1 ,\{\omega_{b_1},\ldots,\omega_{b_i},\ldots,\omega_{b_n}\} \rangle \vdash \langle q_2,\{\omega_{b_1},\ldots,m : \omega_{b_i},\ldots,\omega_{b_n}\}\rangle \iff \langle q_1, b_i \ll m, q_2 \rangle  \in \delta $ 
...
\begin{definition}[Ejecución de un AFCA] Una ejecución es una secuencia $\langle q, \Omega \rangle \vdash \langle q_1, \Omega_1 \rangle \vdash \ldots \vdash \langle q_n, \Omega_n \rangle$ donde $\langle q, \Omega \rangle$ es la configuración inicial, $
\langle q_n, \Omega_n \rangle$ es una configuración final y $ \forall i \in [1, \ldots, n], 
\langle q_i, \Omega_i \rangle$ es una configuración válida (una configuración que no es de deadlock, mensajes huérfanos ni recepción no especificada).
\end{definition}
...
\begin{itemize}
    \item Transiciones de comunicación interna: $b_0 \ll m_1$ y $b_0 \gg m_1$
...
Dados $\mathcal{P}$ un conjunto de participantes, $\mathcal{M}$ un conjunto de mensajes y $A_\mathcal{P} = \{A_{p} \ | \ p \in \mathcal{P}\}$ un conjunto finito de autómatas finitos de comunicación asincrónica $A_{p_i}= \langle Q_{p_i}, B_{p_i}, \mathcal{C}_{p_i}. \Sigma_{p_i}, \delta_{p_i}, q_{0{p_i}}, F_{p_i} \rangle$, diremos que $A_\mathcal{P}$ es \emph{compatible} si se satisface que para todos $p_i, p_j \in \mathcal{P}$, $\Sigma_{p_i} \cap \Sigma_{p_j}= \emptyset$ y $B_{p_i} \cap B_{p_j} = \emptyset$.
\end{definition}

\begin{definition}[Composición]
\label{def:composicion}
Dados $\mathcal{P}$ un conjunto de participantes, $\mathcal{M}$ un conjunto de mensajes y $A_\mathcal{P} = \{A_{p} \ | p \in \mathcal{P}\}$ un conjunto finito de autómatas finitos de comunicación asincrónica compatibles $A_{p_i}= \langle Q_{p_i}, B_{p_i}, \mathcal{C}_{p_i}. \Sigma_{p_i}, \delta_{p_i}, q_{0{p_i}}, F_{p_i} \rangle$.
% Para cada par de participantes distintos $p_i,p_j \in \mathcal{P}$ llamamos  al conjunto de canales que comparten para intercambiar mensajes y $\Sigma_{p_i \rightarrow p_j}$ al conjunto de etiquetas de comunicación entre ambos.
Luego, definimos la composición $A_\mathcal{P} = ||_{1..n} A_{p_i}$ como $A= \langle Q, B, \mathcal{C}, \Sigma, \delta, q_{0}, F \rangle$ tal que:
\begin{itemize}
    ...
    \item $B_\mathcal{P} = \bigcup_{p_i \in \mathcal{P}} B_{p_i} \cup \{ \omega_c \ | \ c \in  \mathcal{C}_{p_i \rightarrow p_j}\}$, dónde $\mathcal{C}_{p_i \rightarrow p_j}= \{c \in \mathcal{C}_{p_i} \cap \mathcal{C}_{p_j} \}$ (i.e. el conjunto de nombres de buffers del autómata resultante se compone de los buffers de cada autómata participante en la composición, junto con uno nuevo por cada canal compartido entre cada par de autómatas, dichos canales son aquellos mediante los cuales los autómatas a componer intercambian mensajes entre sí),    
    \item $\mathcal{C}_\mathcal{P} = \bigcup_{p_i \in \mathcal{P}} \mathcal{C}_{p_i} \setminus \mathcal{C}_{p_i \rightarrow p_j}$,    
    ...
    \begin{enumerate}[1)]
        ...
        \end{enumerate}
donde $\Sigma_\mathit{p_i \mapsto p_j} =\{ \langle p_i,p_j,c \rangle \ | \ p_i, p_j \in \mathcal{P}, \  c \in \mathcal{C}_{p_i \rightarrow p_j}\}$
...
\item $\delta_{\mathcal{P}\mathit{Ex}} \subseteq Q_{\mathcal{P}} \times \Sigma_{\mathcal{P}\mathit{Ex}} \times Q_{\mathcal{P}}$ es la relación de transición de comunicación externa, tal que que se satisface la siguiente fórmula: $$(\forall \langle q, \sigma, q' \rangle \in \delta_{\mathcal{P}\mathit{Ex}}) (\exists p_i \in \mathcal{P})(\exists q_{p_i},q'_{p_i} \in Q_{p_i}, \sigma \in \Sigma_{p_i}) (\langle q_{p_i},\sigma,q'_{p_i} \rangle \in \delta_{p_i Ex})$$

\item $\delta_{\mathcal{P}\mathit{Buff}} \subseteq Q_{\mathcal{P}} \times \Sigma_{\mathcal{P} \mathit{Buff}} \times Q_{\mathcal{P}}$ es la relación de transición de comunicación interna tal que se satisface la siguiente fórmula:
$$
\begin{array}{l}
...
\label{afca-compo-parcial}
El objetivo principal de esta clase de autómatas es posibilitar la composición parcial de componentes de un sistema, como idea de que sirva de reconfiguración dinámica del mismo. Esto debiera visibilizarse a partir de poder, dado un conjunto de AFCAs, tomar un subconjunto y componerlo, y que el resultado siga siendo un conjunto de AFCAs que puede ser compuesto a su vez. En la Fig.~\ref{a_i} vemos un conjunto de tres AFCA compatibles para la composición. Tomando este mismo conjunto podemos elegir componer solo dos y nos seguiría quedando un conjunto de AFCAs compatibles, como podemos ver en la Fig.~\ref{A-parcial} con el autómata compuesto $\mathcal{A}_{PR}$ y el autómata $S$.
...
\begin{enumerate}
    ...
    \item $[q_0]=q_0$
    \item $[\delta]= \{[q] \xrightarrow{\sigma} [q']\ |\ [q], [q'] \in [Q] \land \sigma \in \Sigma_{EX}\}$, dos estados $[q],[q'] \in [Q]$ están relacionados en $[\delta]$ si son clases de equivalencia relacionadas. 
\end{enumerate}
\end{definition}

\begin{ejemplo}[Interfaz de comunicación de un AFCA] 
...
Las siguientes mCFSMs son el resultado de proyectar la interfaz de comunicación (Def.~\ref{def:interfazcom}) de los AFCAs de la Fig.~\ref{a_i}, donde $M_1, M_2$ y $M_3$ representan las interfaces de comunicación externa de $\mathcal{A}_1, \mathcal{A}_2$ y $\mathcal{A}_3$, respectivamente.
\begin{figure}[H]
\begin{center}
\begin{tikzpicture}[->, thick]
 \node[state,initial] (q_0)   {$[q_0]$}; 
