...
\cnote{Reescribir el párrafo hablando de la figura y mencionando las subfiguras cuando es necesario.}
...
\begin{itemize}
    ...
    \item $\widehat{\Delta}= \{\langle \langle q_i, \Omega \rangle, \widehat{\sigma},\langle q_j, \Omega' \rangle \rangle | q_i, q_j \in Q, \widehat{\sigma} \in \{B \times \{\gg, \ll\} \times \mathcal{M}\} \land \exists \sigma \in \Sigma$ tal que  $[q_i]_m \xrightarrow{\sigma} [q_j]_m \}$\cnote{Agregar aristas de comunicación externa.}. Definimos $\widehat{\Delta}$ como la relación de transición de $\widehat{\mathcal{A}}$. Un estado de $\widehat{q_j}$ es alcanzable desde otro $\widehat{q_i}$ si la clase de equivalencia $[q_j]_m$ es alcanzable desde $[q_i]_\mathcal{M}$.
\end{itemize}
\end{definition}

\cnote{Parrafo explicando qué se demuestra y qué no. Usar figuras para ejemplificar.}
...
\subfloat[$\mathcal{A}$, composición de los AFCAs $\mathcal{A}_i$]{
\begin{tikzpicture}[->, thick]
...
\subfloat[Proyección de comunicación interna de $\mathcal{A}$]{
\centering
\begin{tikzpicture}[->, thick]
