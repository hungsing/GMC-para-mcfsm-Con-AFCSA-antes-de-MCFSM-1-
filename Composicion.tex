...
\begin{enumerate}
\item $\delta_{\mathcal{P}\mathit{Int}} \subseteq Q_{\mathcal{P}} \times \Sigma_{\mathcal{P} Int} \times Q_{\mathcal{P}}$ donde $\forall \langle q, \sigma, q' \rangle \in \delta_{\mathcal{P}\mathit{Int}} \exists q_{p_i},q'_{p_i} \in Q_{p_i} \sigma \in \Sigma_{p_i}$ tal que $\langle q_{p_i},\sigma,q'_{p_i} \rangle \in \delta_{p_i Int}$ es la relación de transición interna
\item $\delta_{\mathcal{P}\mathit{Ex}} \subseteq Q_{\mathcal{P}} \times \{\mathit{In}(e,m), \mathit{Out}(e,m) | e \in \Sigma_{\mathcal{P}\mathit{Ex}} \land m \in \mathcal{M}\} \times Q_{\mathcal{P}}$ donde $\forall \langle q, \sigma, q' \rangle \in \delta_{\mathcal{P}\mathit{Ex}} \exists q_{p_i},q'_{p_i} \in Q_{p_i}, \sigma \in \Sigma_{p_i}$ tal que $\langle q_{p_i},\sigma,q'_{p_i} \rangle \in \delta_{p_i Ex}$, es la relación de transición externa y 

\item $\delta_{\mathcal{P}\mathit{Buff}} \subseteq Q_{\mathcal{P}} \times \Sigma_{\mathcal{P} Buff} \times Q_{\mathcal{P}}$ donde $\forall \langle q, \sigma, q' \rangle \in \delta_{\mathcal{P}\mathit{Buff}} \exists q_{p_i},q'_{p_i} \in Q_{p_i}, \sigma \in \Sigma_{p_i}$ tal que $\langle q_{p_i},\sigma,q'_{p_i} \rangle \in \delta_{p_i}$. Esto puede ocurrir de dos formas:\begin{inparaenum}\item $\sigma \in \Sigma_{p_i Buff}$ y $\sigma = \omega_b \gg/\ll m $ con $\omega_b \in B_{p_i}$ o \item $\exists \sigma' \in \Sigma_{p_i Ex}$ de modo que $\sigma' = In(e,m)/Out(e,m)$.
\end{inparaenum} \enote{Quiero decir que la transición de buffer puede haber sido una transición de comunicación externa entre dos autómatas componentes} Además vale que $\forall q_i,q_j \in Q_\mathit{\mathcal{P}}, \ m \in \mathcal{M} \ |	 \ \langle q_i, [\omega_\mathit{b}]_{b \in B_\mathcal{P}} \gg m, q_j \rangle$ y $\langle q_{p_i},\sigma,q'_{p_i} \rangle \in \delta_{p_i Buff}$ $ \delta_\mathit{\mathcal{P}Buff} \iff \exists$ las configuracioes $c_i = \langle q_i, [\omega_\mathit{b},...,\omega_\mathit{b}:m, ..., \omega_\mathit{b}] \rangle, c_j =\langle q_j, [\omega_\mathit{b},...,\omega_\mathit{b}, ..., \omega_\mathit{b}] \rangle$ y $c_i \vdash c_j$ \enote{Está bien expresado esto? Me hace un poco de ruido esto de hablar de configuraciones en la definición}
\end{enumerate}
