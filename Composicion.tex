...
$$
\begin{array}{l}
(\forall \langle q, \sigma, q' \rangle \in \delta_{\mathcal{P}\mathit{Buff}})(\\
\qquad
\begin{array}{l}
(\exists p_i \in \mathcal{P})(\exists \widehat{q}, \widehat{q'} \in Q_{p_i}, \sigma \in \Sigma_{{p_i} \mathit{Buff}})(\sigma = \omega_b \{\gg, \ll\} m \land \omega_b \in B_{p_i}) \lor\\
(\exists p_i, p_j \in \mathcal{P})(p_i \not= p_j \land (\\
\qquad (\exists \mathit{Out}(c_{{p_i, p_j}_n}, m) \in \Sigma_{p_i \mathit{Ex}})(\sigma = \omega_{c_{{p_i, p_j}_n}} \ll m) \lor\\
\qquad (\exists \mathit{In}(c_{{p_j, p_i}_n}, m) \in \Sigma_{p_i \mathit{Ex}})(\sigma = \omega_{c_{{p_j, p_i}_n}} \gg m))))
\end{array}
\end{array}
 $$
