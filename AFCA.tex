...
\item $\Sigma_\mathit{Ex} = \{\sigma \in \{\mathit{In}(c,m),\mathit{Out}(c,m)\} \} $ un conjunto de etiquetas dónde $c \in \mathcal{C}$ es el canal a través del cual se resuelve la misma y $m \in \mathcal{M}$ es el mensaje .
