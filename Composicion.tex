...
\begin{itemize}
    ...
    \item Por lo tanto, $q_i \in Q_{M_i}$, por definición de Interfaz de Comunicación de AFCA. Y $q_i \in Q_{M_i} \iff \langle \langle q_i,..,q_r \rangle, \overrightarrow{\Omega} \rangle \in Q_M$ dónde $M$ es el LTS que representa la semántica del communicating system.
    
    \item[\textbf{Estados} $\impliedby$]
    \item Sea $M$ el LTS que representa la semántica el communicating system $\langle \langle q_1,..,q_m \rangle, \overrightarrow{\Omega} \rangle \in Q_M  \iff q_i \in Q_{M_i} \iff q_i \in Q_i$ y ya establecimos $\forall q_i \in  Q_{M_i}$ vale que $\exists [q_i]_m$. Por definición de composición $q_i \in Q_i \iff \iff \langle q_1, q_2,.., q_i \rangle \in Q $. Como ya establecimos que por la composición $\exists [q_i]_m$ para $\mathcal{A}_i$ $\iff \exists [\langle q_1, q_2,.., q_i \rangle]_m$ para $\mathcal{A}$, y por definición de proyección $\exists \langle \langle q_1, q_2,.., q_m \rangle, \overrightarrow{\Omega} \rangle \in \hat{Q}$
    
    \item[\textbf{Buffers} $\implies$] 
    \item Por definición de proyección de comunicación, los buffers representados en $\hat{\mathcal{A}}$ son buffers de $\mathcal{A}$. Por definición de composición, estos buffers surgen de los buffers $B_i$ de los AFCA componentes más los canales de comunicación internalizados. Es decir los buffers de tipo $(a_ka_j)_l \in \mathcal{C}_i$. Como los AFCA del conjunto $\{\mathcal{A}_i\}_{i \in I}$ carecen de operaciones de comunicación interna, los buffers que se modifican entre estados de $\hat{\mathcal{A}}$ son únicamente los de canales internalizados. Por definición de interfaz de comunicación de AFCA, son los mismos canales que aparecen en las mCFSM $M_i$ y por lo tanto son los mismos que aparecen en los estados del lts $M$.

    \item[\textbf{Buffers} $\impliedby$]
    \item Los $\Omega$ que se modifican entre estados de $M$ son canales de comunicación externa del conjunto de mCFFSM $\{M_i\}_{i \in I}$. Para toda mCFSM del conjunto $M_i$, un canal de comunicación está pertenece a su cojunto de canales $\mathcal{C}_{M_i}$ si y solo si, son canales en el AFCA correspondiente. Por definición de composición estos canales se internalizan como buffers de comunicación interna en $\mathcal{A}$. Por definición de proyección de comunicación interna, estos buffers están en los estados del lts $\hat{\mathcal{A}}$. Como los $\mathcal{A}_i$ no tienen operaciones de comunicación interna. Estos los buffers que se modifican entre estados de $\hat{\mathcal{A}}$ son únicamente los canales internalizados. Por lo tanto los canales de los estados de $M$ son exactamente los mismos que se modifican en los estados de $\hat{\mathcal{A}}$.
    
    \item[\textbf{Transiciones}]
    \item Ya demostramos que los estados de $\hat{\mathcal{A}}$ y $M$ son equivalentes. Llamemos $R \subseteq \hat{Q} \times P$ a esta relación de equivalencia donde $P$ es el conjunto de estados de $M$ y para todo un par $\langle q, p \rangle \in R$ sii son $q$ y $p$ son estados equivalentes. Queremos probar que $R$ es una bisimulación.
    
    \item Sean $q \in \hat{Q}, p \in P $, para todo $\sigma \in \hat{\Sigma}$, $q \xrightarrow{\sigma} q'$, $\sigma$ es una transición sobre un buffer  $(a_ka_j)_i$ de la forma $\langel q, (a_ka_j)_i \{\ll, \gg\} m, q'$. Como los estados son equivalentes y los buffers modificados en $\hat{\mathcal{A}}$ son los canales de $M$, podemos deci que existe un $\sigma' \in Sigma_M$ tal que $p \xrightarrow{\sigma'} p'$ y $\langle q', p' \rangle \in R$
    
    \item Simétricamente podemos decir que $\forall \sigma' \in \Sigma_M$ tal que $p \xrightarrow{\sigma'} p'$, $\sigma'$ es una operación sobre un canal de comunicación. Como los estados son equivalente, los canales comunicación de $M$ están en $\hat{\mathcal{A}}$ como buffers y de hecho son los únicos buffers que se modifican de estado a estado, sabemos que hay una transición $\sigma \in \hat{\Sigma}$ tal que $q \xrightarrow{\sigma} q'$ y $\langle q', p' \rangle \in R$.
\end{itemize}
Como probamos que el conjunto de estados de $\hat{\mathcal{A}}$ y $M$ son equivalentes, y lo mismo para las transiciones, podemos afirmar que $\hat{\mathcal{A}}$ y $M$ son bisimilares. 
