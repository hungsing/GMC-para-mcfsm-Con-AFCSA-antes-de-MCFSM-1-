...
\begin{itemize}
    \item $S= \langle \{q_0,..,q_4\},[b_0],\{sr_1,rs_1\}, \Sigma_A, \delta_A, q_0, \{q_4\} \rangle$. Donde $\Sigma =\{wait, b_0 \ll m_1,b_0 \gg m_1,Out(sr_1,c), In(rs_1,b) \}$
    \item $R=\langle \{q_0,q_1,q_2\},[],\{sr_1,rs_1\}, \Sigma_A, \delta_A, q_0, \{q_4\} \rangle $
\end{itemize} 
\begin{tikzpicture}[->, thick]
 ...
 \draw[]        
        ...
        (q_0) edge[left] node{$b_0 \ll m_1$} (q_2)
        (q_1) edge[right] node{$wait$} (q_3)
        (q_2) edge[above] node{$b_0 \gg m_1$} (q_3)
...
\begin{tikzpicture}[->, thick]
 ...
 \node[state] (q_1) [right= 1.8cm of q_0 ] {$q_1$};
  \node[state,accepting] (q_2) [right= 1.8cm of q_1] {$q_2$};
 \draw[]        
        (q_0) edge[above] node{$Out(sr_1,b)$} (q_1)
        (q_1) edge[above] node{$In(sr_1,c)$} (q_2)
        ;
\end{tikzpicture}
