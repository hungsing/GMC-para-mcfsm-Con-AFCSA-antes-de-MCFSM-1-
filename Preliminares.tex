...
\end{itemize}
Dados dos estados $p, q \in Q $, $p$ es débilmente bisimilar sobre $\Sigma'$ a $q$, denotado como $p \sim_{\Sigma'} q$, si existe una bisimulación débil $R$ tal que $\langle p, q \rangle \in R$.
\end{definition}
...
\begin{itemize}
  ...
  \item $C \subseteq \left\{ pq \in \mathcal{P}^2 \left|\right. p \not= q\right\}$ es un conjunto de canales
