...
\begin{proof}Primero necesitamos ver que para cada estado de $\hat{\mathcal{A}}$ existe un estado equivalente en $C$, y viceversa. Es decir $(\exists \hat{q} \in \hat{Q} \iff \exists p \in Q_C) \land q \approx p$.

Cada transición $\hat{sigma} \in \hat{\Delta}$ es un movimiento en un buffer $b \in B_{\mathcal{A}}$ tal que existe un canal $c \in \mathcal{C}$ que le corresponde. 
\end{proof}
\end{prop}
