...
\end{tikzpicture} 
\end{center}
\caption{Autómatas finitos de comunicación asincrónica $P$, $R$, $S$.}
\label{a_i}
\end{figure}
...
\begin{tikzpicture}[->, thick]
 ...
 \node[state] (q_12) [above = of q_3 ] {$q_{111}$};
  \node[state] (q_4) [right = 1.5 of q_3 ] {$q_{121}$};
  \node[state] (q_5) [below=2.5  of q_0] {$q_{001}$};
  \node[state] (q_6) [right = 1.5cm of q_5] {$q_{201}$};
   \node[state] (q_7) [below =  of q_2 ] {$q_{301}$};
  \node[state] (q_11) [below = of q_7] {$q_{211}$};
  \node[state] (q_8) [right = 1.5 of q_7 ] {$q_{311}$};
  \node[state] (q_9) [right = 1.5 of q_8 ] {$q_{321}$};
  \node[state] (q_13) [below = of q_9 ] {$q_{411}$};
 ...
 \draw[]        
		...
		(q_12) edge[right] node{$PR \gg a$} (q_4)
...
\begin{tikzpicture}[->, thick]
 ...
 \node[state] (q_1) [right= 1.5cm of q_0 ] {$q_{01}$};
 \node[state] (q_2) [right = 2 of q_1 ] {$q_{11}$};
 \node[state] (q_3) [below = 1.5 of q_2 ] {$q_{12}$};
  \node[state] (q_4) [below = of q_3 ] {$q_{32}$};
  \node[state] (q_5) [below=  of q_1] {$q_{10}$};
  \node[state] (q_6) [below = 1.5cm of q_5] {$q_{30}$};
  \node[state] (q_7) [below = 1.5 of q_6 ] {$q_{31}$};
  \node[state] (q_8) [below = of q_0 ] {$q_{20}$};
  \node[state] (q_9) [below = of q_8 ] {$q_{21}$};
 \node[state,accepting] (q_10) [below= 1.5cm of q_4 ] {$q_{42}$};
 \draw[]        
        (q_0) edge[above] node{$int_r$} (q_1)
        (q_1) edge[above] node{$PR \ll a$} (q_2)
		(q_2) edge[right] node{$PR \gg a$} (q_3)
        (q_3) edge[right] node{$in(SP,b)$} (q_4) 
        (q_0) edge[right] node{$PR \ll a$} (q_5)
        (q_5) edge[right] node{$in(SP,b)$} (q_6)
        (q_6) edge[left] node{$int_r$} (q_7)
        (q_7) edge[right] node{$PR \gg a$} (q_4)
        (q_0) edge[left] node{$in(SP,b)$} (q_8)
        (q_8) edge[left] node{$int_r$} (q_9)
        (q_9) edge[left] node{$PR \ll a$} (q_7)
        (q_4) edge[right] node{$int_p$} (q_10)
        ;
\end{tikzpicture}
\
\begin{tikzpicture}[->, thick]
