...
\begin{figure}[h]
\begin{example}[\text{Proyección de Comunicación Interna de AFCA}]
\label{ex:pci}
\subfloat[a][AFCAs $\mathcal{A}_1$, $\mathcal{A}_2$, $\mathcal{A}_3$]{
\begin{tikzpicture}[->, thick]
 \node[state,initial] (q_0)   {$q_0$}; 
...
\subfloat[b][$\mathcal{A}$, composición de los AFCAs $\mathcal{A}_i$]{
\begin{tikzpicture}[->, thick]
...
\subfloat[c][Proyección de comunicación interna de $\mathcal{A}$]{
\begin{tikzpicture}[->, thick]
...
\begin{figure}[h]
\begin{example}[Semántica de un CS de mCFSMs]
\label{ex:CS}
\subfloat[1][Las mCFSM $M_1$, $M_2$ y $M_3$, correspondientes a los AFCAs del Ejemplo \ref{ex:pci}]{
\begin{tikzpicture}[->, thick]
 \node[state,initial] (q_0)   {$q_0$}; 
...
\subfloat[2][El LTS $M$ representa la semántica del CS de las mCFSM]{
