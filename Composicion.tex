...
(\exists p_i, p_j \in \mathcal{P})(p_i \neq p_j) \land (\forall p_k \in \mathcal{P})((p_k \neq p_i \land p_k \neq p_j) \implies q_k = q'_k) \land \\ (\exists \widehat{q}, \widehat{q'} \in Q_{p_i} \lor Q_{p_j}) \land (q_i = \widehat{q} \land q'_i = \widehat{q'}) \land (\\ \qquad (\exists \mathit{Out}(c_{{p_i, p_j}_n}, m) \in \Sigma_{p_i \mathit{Ex}})(\sigma = \omega_{c_{{p_i, p_j}_n}} \ll m) \lor\\
