...
Sea $\{ \mathcal{A}_i \}_{i \in I}$ un conjunto de AFCA $\{ \mathcal{A}_i \} = \langle Q, B, \mathcal{C}, \delta, q_0, F \rangle$ tal que $\delta_{Buff} =  \emptyset$. Llamamos $||{A_i}_{i \in I}$ a la composición total. La misma tiene la forma $\mathcal{A} = \langle Q, B, \mathcal{C}, \delta, q_0, F \rangle$ donde $\delta_{Ex} =  \emptyset$. Definimos $\Pi': AFCA \rightarrow \mathcal{L}'$ tal que
\begin{itemize}
    \item $\forall $ Cada estado de $n$ de $\mathcal{L}'$ corresponde a una configuración $\langle q, \Omega \rangle$ de $||{A_i}_{i \in I}$ y
    \item Un estado $n$ es alcanzable desde otro estado $n'$ sii $\exists \langle q, \sigma, q' \rangle \in \delta_{||{A_i}_{i \in I}}$ con $\sigma \in \Sigma_{Buff}$ y es de la forma $w_b\{\gg, \ll\}m$ 
\end{itemize}
\end{definition}
...
Consideremos los autómatas representaos por el communicating system del ejemplo 2
    \centering
...
\begin{tikzpicture}[->, thick][scale=0.20]
 ...
 \node[state] (q_3) [right = 1.5 of q_2 ] {$q_{111}$};
  \node[state] (q_4) [below=  of q_0] {$q_{001}$};
  \node[state] (q_5) [right = 1.5 of q_4 ] {$q_{201}$};
  \node[state] (q_6) [right = 1.5 of q_5 ] {$q_{301}$};
 \node[state,accepting] (q_7) [right= 1.5cm of q_6 ] {$q_{311}$};
 \draw[]        
        (q_0) edge[above] node{$PR \ll a$} (q_1)
        (q_1) edge[above] node{$PR \gg a$} (q_2)
        (q_2) edge[above] node{$SP \ll b$} (q_3)
        (q_0) edge[left] node{$SP \ll b$} (q_4)
        (q_4) edge[below] node{$SP \gg b$} (q_5)
        (q_5) edge[below] node{$PR \ll a$} (q_6)
        (q_6) edge[below] node{$PR \gg a$} (q_7)
        (q_3) edge[right] node{$SP \gg b$} (q_7)
        ;
\end{tikzpicture}
\\
\begin{tikzpicture}[->, thick][scale=0.20]
