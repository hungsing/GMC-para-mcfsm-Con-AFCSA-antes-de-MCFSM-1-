...
% $\forall \hat{\sigma} \in \hat{\Delta} \exists \langle q,a_ka_j{\ll,gg}m,q' \in \delta_{\mathcal{A}}$ con $a_ka_j \in B_{\mathcal{A}}$. $a_ka_j \in B_{\mathcal{A}} \iff \exists a_ka_j \in \mathcal{C}$
    ...
    \item[$\implies$] $\overrightarrow{\Omega}$ es la secuencia de buffers $B$ con sus respectivos contenidos en cada estado. Por la definición de composición [Def~\ref{def:comp}] $B$ contiene los buffers que eran canales de comunicación externa $(a_ka_j)_n \in \mathcal{C}_{\mathcal{A}_i}$, más los buffers propios de cada componente $b \in B_{\mathcal{A}_i}$. Por definición de $\hat{\Delta}$, un estado $\langle q', \overrightarrow{\Omega} \rangle$ es alcanzable desde otro $\langle q, \overrightarrow{\Omega} \rangle$, $(q,q' \in Q$, si existe una transición de comunicación interna $\langle q,(a_ka_j)_n \ll m ,q' \rangle \in \delta_{\mathcal{A}}$ o $\langle q,(a_ka_j)_n \ll m ,q' \rangle \in \delta_{\mathcal{A}}$. Sabemos por la definición de proyección de comunicación interna (Def~\ref{def:pci}) que los autómatas de $\{\mathcal{A}_i\}_{i \in I}$ no tienen transiciones de comunicación interna. Por lo tanto podemos afirmar que los buffers que cambian su contenido en cada estado de $\hat{\mathcal{A}}$ son únicamente los de la forma $(a_ka_j)_n$. Ya probamos que las transiciones expresadas $\hat{A}$ son semánticamente equivalentes a las de $C$. Entonces podemos decir que los buffers que se modifican en $\hat{A}$ son los mismos canales que se modifican en $C$.
    ...
    \item  Para cada par de estados $\hat{p}, \hat{q} \in \hat{Q}$ existe una transición de comunicación $\hat{\alpha} \in \hat{\Delta}$ tal que $\hat{p} \xrightarrow{\hat{\alpha}} \hat{q}$. Y para cada par de estados $p, q \in Q_C$ existe una transición $\alpha \in \Delta_C$ tal que $p \xrightarrow{\alpha} q$. Como $alpha \approx \hat{\alpha}$, lo único que se modifica de estado a estado son buffers/canales que son los mismos y cambian del mismo modo en ambos LTS podemos decir que los estados de $\hat{\mathcal{A}}$ reflejan lo mismo que los de $C$, por lo tanto son bisimilares.
\end{itemize}
