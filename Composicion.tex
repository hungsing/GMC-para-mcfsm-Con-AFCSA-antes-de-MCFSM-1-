...
\centering
\xymatrix{   
	\{\mathcal{A}_i\}_{i \in I} \ar[rr]_{\Pi} \ar[d]_{||} & & \{C_i\}_{i \in I}  \ar[dl]_{semantica}  \\
...
\begin{itemize}
    \item Definimos la clase de equivalencia de un AFCA $[q]_{Buff} = \{q'| q \xrightarrow{\sigma} q', \sigma \in \Sigma_{\mathit{Int}} \}$ y $[q] \xrightarrow{\epsilon} [q']$ sii $\exists \hat{q} \in [q] \hat{q'} \in [q'] | \hat{q} \xrightarrow{\epsilon} \hat{q'}$. Es decir que para todo estado $q$ su clase de equivalencia incluye a todo otro estado $q'$ que esté conectado con $q$ por una transición interna.
    \item $ \hat{Q} = \{ [q]_{Buff}| q \in Q_{\mathcal{A}} \}$ Cada estado del autómata resultante es una clase de equivalencia de los estados del autómata original.
    \item $\hat{q_0} = [q_0]$ donde $q_0$ es el estado inicial de $\mathcal{A}$.
    \item $\hat{\delta} = \{\langle q, \delta_i, q' \rangle | \delta_i \in \delta_{\mathit{\mathcal{A} Buff}} \land \delta_i = \langle q_i, \sigma, q_j \rangle \land \q_i, q_j \in Q_{\mathcal{A}} \land \sigma \in \Sigma_{\mathcal{A}} \}$ donde $q = [q_i]_{Buff} \land q' = [q_j]_{Buff}$. Un estado de $\hat{Q}$ es alcanzable desde otro si existe una transición de comunicación interna en el autómata $\mathcal{A}$ 
