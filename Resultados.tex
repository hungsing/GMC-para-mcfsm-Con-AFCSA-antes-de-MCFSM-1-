...
\begin{figure}[H]
\subfloat[Composición de los AFCAs de la Subfig.~\ref{a_i}]{
\label{A}
\begin{tikzpicture}[->, thick]
...
\subfloat[Proyección de comunicación interna del AFCA de la Subfig.~\ref{A}.]{
\label{hatA}
\begin{tikzpicture}[->, thick]
...
\caption{Composición de AFCAs y la proyección de su comunicación interna.}
\label{ex:pci}
\end{figure}

\begin{figure}[H]
\subfloat[1][mCFSM $M_1$, $M_2$ y $M_3$, correspondientes a los AFCAs del Subfig.~\ref{a_i}]{
\label{m_i}
\begin{tikzpicture}[->, thick]
 \node[state,initial] (q_0)   {$q_0$}; 
...
\subfloat[2][LTS $M$ del CS formado por las mCFSMs de la Subfig.~\ref{m_i}.]{
\label{M}
...
\caption{mCFSMs de los AFCAs de la Fig.~\ref{ex:pci} y su semántica.}
\label{ex:CS}
\end{figure}
